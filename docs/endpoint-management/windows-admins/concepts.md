...
| [Users](#users) | AD Users and Computers (the user side) | Identity directory that will sync from Kanidm (planned identity backend) or added manually. Assign console roles, pair identities to assets, view per-user policy resolution. |
...
| [Server Administration](#server-admin) | AD Sites & Services + Intune tenant settings | Tenant-wide configuration of the Pluris console itself: planned Kanidm connection, NATS endpoints, certificate authority, console roles, audit log retention. |
...
| **On Linux** | Planned: Kanidm IDM (LDAP-compatible) + system PAM/SSSD on enrolled Linux hosts. |
...
| **Empty state** | _"No users yet"_ — Connect Kanidm (planned) or add a user manually. Once present, users can be targeted by policies and bound to assets. |
